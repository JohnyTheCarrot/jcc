...
#define NOT_APPLICABLE()                                                       \
  {                                                                            \
    std::cerr << "NOT_APPLICABLE HIT AT " << __FILE__ << ":" << __LINE__       \
...
#define TOSTRING_FIELD_NODE(fieldName, field)                                  \
  ss << tabsChildren << fieldName ": " << (field).ToString(depth + 1)          \
     << std::endl;

#define TOSTRING_FIELD_ENUM(fieldName, field)                                  \
  ss << tabsChildren << fieldName ": " << magic_enum::enum_name(field)         \
     << std::endl;

#define TOSTRING_FIELD_BOOL(fieldName, value)                                  \
  ss << tabsChildren << fieldName ": " << ((value) ? "true" : "false")         \
     << std::endl;
...
struct AstNode {
  ...
  AstNode(AstNodeType type, int hierarchyLevel)
      : _type{type}, _span{}, _hierarchyLevel{hierarchyLevel} {};
  ...
  [[nodiscard]] virtual std::string ToString(size_t depth) const = 0;
