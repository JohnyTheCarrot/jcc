...
namespace parsing {
    std::optional<AstFunctionDefinition> AstFunctionDefinition::Parse(Parser &parser) {
        ...
        if (nextToken._type == TokenType::Semicolon
            || nextToken._type == TokenType::Comma
        ...
        return AstFunctionDefinition{ span, std::move(*specifiers), std::move(*declarator), std::move(*compoundStatement) };
    }

    std::string AstFunctionDefinition::ToString(size_t depth) const {
        TOSTRING_FIELDS(AstFunctionDefinition, depth, {
