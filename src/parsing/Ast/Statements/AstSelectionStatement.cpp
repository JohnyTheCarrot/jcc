...
namespace parsing {
	std::optional<AstSelectionStatement> AstSelectionStatement::Parse(Parser &parser) noexcept {
		...
		if (!keywordIf) {
			...
			if (!expression)
				parser.Error(leftParen._span, "Expected to be followed by expression");
			...
			if (!statement)
				parser.Error(rightParen._span, "Expected to be followed by statement");
		...
		if (!condition)
			parser.Error("If statement is missing _condition");

		const Token &rightParen{parser.ExpectToken(TokenType::RightParenthesis, span)};

		AstNode::Ptr ifBody{AstStatement::Parse(parser)};
		if (!ifBody)
			parser.Error(rightParen._span, "If statement is missing body");
		...
		if (keywordElse) {
			...
			if (!elseBody)
				parser.Error(keywordElse->_span, "Else statement is missing body");
	...
	std::string AstSelectionStatement::ToString(size_t depth) const {
		TOSTRING_FIELDS(AstSelectionStatement, depth, {
			...
			if (_elseBody)
				TOSTRING_FIELD_NODE("elseBody", *_elseBody);
