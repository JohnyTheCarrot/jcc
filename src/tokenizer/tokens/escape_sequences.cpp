...
namespace jcc::tokenizer::escape_sequences {
	...
	[[nodiscard]]
	int HexadecimalEscapeSequence(CharIter &charIter, SpanMarker const &backslashMarker) {
		...
		if (charIter == CharIter::end()) {
			Span span{
			        charIter.GetFileName(), backslashMarker, charIter.GetCurrentSpanMarker(),
		...
		if (auto const firstDigit{GetHexadecimalDigit(charIter->m_Char)}; firstDigit.has_value()) {
			...
			throw FatalCompilerError{Diagnosis::Kind::TK_CharHexNoDigits, std::move(span), charIter->m_Char};
		}
		...
		while (++charIter != CharIter::end()) {
			...
			if (auto const digit{GetHexadecimalDigit(charIter->m_Char)}; digit.has_value()) {
				...
				if (++digitIdx == N_INT_HEX_DIGITS) {
					...
					throw FatalCompilerError{Diagnosis::Kind::TK_EscapeSequenceValueTooLarge, std::move(span)};
				}
	...
	ValidateEscapeSequenceSize(compiler_data_types::Char32::type result, ConstantPrefix prefix, Span &span) {
		if (auto const [min, max]{GetConstantPrefixRange(prefix)}; result < min || result > max)
			throw FatalCompilerError{Diagnosis::Kind::TK_EscapeSequenceValueTooLarge, std::move(span)};

		return result;
	}

	compiler_data_types::Char32::type TokenizeNoSizeCheck(CharIter &charIter, SpanMarker const &backslashMarker) {
		if (charIter == CharIter::end()) {
			SpanMarker const &currentMarker{charIter.GetCurrentSpanMarker()};
			Span              span{
                    charIter.GetFileName(), currentMarker, currentMarker, charIter.GetInput()->tellg(),
                    charIter.GetInput()
            };
		...
		if (auto const simpleEscapeSequenceResult{SimpleEscapeSequence(c)}; simpleEscapeSequenceResult.has_value()) {
		...
		Span span{
		        charIter.GetFileName(), backslashMarker, charIter.GetCurrentSpanMarker(), charIter.GetInput()->tellg(),
		        charIter.GetInput()
		};
		...
		throw FatalCompilerError{Diagnosis::Kind::TK_UnknownEscapeSequence, std::move(span), c};
	}

	compiler_data_types::Char32::type
	Tokenize(CharIter &charIter, SpanMarker const &backslashMarker, ConstantPrefix prefix) {
