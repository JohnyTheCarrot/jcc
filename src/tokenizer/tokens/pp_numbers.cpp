...
namespace jcc::tokenizer::pp_numbers {
	...
	bool GatherDigit(CharIter &charIter, Span &span, std::string &number) {
		switch (charIter->m_Char) {
			case '\'':
				...
				if (charIter == CharIter::end() ||
				    (!utils::IsNonDigit(charIter->m_Char) && std::isdigit(charIter->m_Char)))
					throw FatalCompilerError{Diagnosis::Kind::TK_PreprocessingNumberInvalid, std::move(span)};
		...
		if (!std::isdigit(charIter->m_Char) && !utils::IsNonDigit(charIter->m_Char)) {
			return false;
		}
	...
	Token Tokenize(CharIter &charIter, SpanMarker const &start, bool startsWithDot) {
		...
		Span        span{charIter.GetFileName(), start, start, charIter.GetInput()->tellg(), charIter.GetInput()};

		if (charIter == CharIter::end()) {
			return Token{.m_Value = PpNumber{std::move(number)}, .m_Span = std::move(span)};
		}
		...
		while (charIter != CharIter::end()) {
			if (auto const hasNext{GatherDigit(charIter, span, number)}; !hasNext)
				break;
