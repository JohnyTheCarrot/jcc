...
namespace jcc::tokenizer {
	...
	bool CharIter::Next() {
		if (char next{}; m_Input->get(next).good()) {
			...
			auto &[spanMarker, character, isSentinel]{std::get<value_type>(m_CurrentChar)};
			character  = next;
	...
	CharInfo CharIter::Expect(char c) {
		if (!Next()) {
			auto const lastSpanMarker{std::get<Sentinel>(m_CurrentChar).m_LastSpanMarker};
			Span       span{m_FileName, lastSpanMarker, lastSpanMarker, m_Input->tellg(), m_Input};

			throw FatalCompilerError{Diagnosis::Kind::UnexpectedEOF, std::move(span)};
		}

		if (auto const [spanMarker, character, isSentinel]{std::get<value_type>(m_CurrentChar)}; character != c) {
			Span span{m_FileName, spanMarker, spanMarker, m_Input->tellg(), m_Input};

			throw FatalCompilerError{Diagnosis::Kind::TK_UnexpectedChar, std::move(span), character};
		}
	...
	CharIter &CharIter::operator++() {
		while (true) {
			...
			switch (auto nextChar{std::get<value_type>(m_CurrentChar)}; nextChar.m_Char) {
				case '\r':
					Expect('\n');
	...
	bool CharIter::operator==(CharIter const &other) const {
		if (std::holds_alternative<Sentinel>(m_CurrentChar) && std::holds_alternative<Sentinel>(other.m_CurrentChar)) {
			return true;
		}

		if (std::holds_alternative<CharInfo>(m_CurrentChar) && std::get<CharInfo>(m_CurrentChar).m_IsSentinel &&
		    std::holds_alternative<Sentinel>(other.m_CurrentChar)) {
			return true;
		}

		if (std::holds_alternative<Sentinel>(m_CurrentChar) && std::holds_alternative<CharInfo>(other.m_CurrentChar) &&
	...
	void PrintTo(CharInfo const &charInfo, std::ostream *os) {
		*os << std::format(
		        "CharInfo{{m_Char: {}, m_SpanMarker: {}}}", testing::PrintToString(charInfo.m_Char),
