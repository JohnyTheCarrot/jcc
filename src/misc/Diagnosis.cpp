...
namespace jcc {
    ...
    void Diagnosis::PrintDiagMessage() const {
        switch (m_Kind) {
            ...
            case Kind::TK_CharOutOfRange:
                fmt::print("Character literal is out of range, its value will "
                           "be truncated.");
                break;
            ...
            case Kind::TK_UTFCharMoreThanOneChar:
                fmt::print("A UTF-8, UTF-16, or UTF-32 character literal must "
            ...
            case Kind::TK_DirectiveNotAloneOnLine:
                fmt::print("A preprocessor directive must be the only item on "
                           "a line.");
                break;
            ...
            case Kind::PP_MacroExpectedLParen:
                fmt::print(
                        "Expected left parenthesis as part of macro invocation."
                );
                break;
            ...
            case Kind::PP_MacroTooManyArgs:
                fmt::print("Too many arguments passed to non-variadic "
                           "function-like macro.");
                break;
            ...
            case Kind::PP_UnexpectedMacroInvocationArgumentCount:
                fmt::print("Invalid amount of arguments passed to "
                           "function-like macro.");
                break;
            case Kind::PP_MacroDefinedInTermsOfItself:
                fmt::print("Macros may not be defined in terms of themselves, "
    ...
    void Diagnosis::Print() const {
        ...
        if (m_Span.m_Start.m_LineNumber == m_Span.m_End.m_LineNumber) {
            ...
            OutputHighlight(
                    m_Span.m_Start.m_RealCharacterIndex,
        ...
        OutputLine(m_Span.m_Start.m_LineNumber, line);
        OutputHighlight(
                m_Span.m_Start.m_RealCharacterIndex, std::nullopt,
                static_cast<int>(line.size()), m_Class
        );
        ...
        for (int lineNum{m_Span.m_Start.m_LineNumber + 1};
            ...
            OutputHighlight(
                    std::nullopt, std::nullopt, static_cast<int>(line.size()),
                    m_Class
            );
        }
        ...
        OutputHighlight(
                std::nullopt, m_Span.m_End.m_RealCharacterIndex,
                static_cast<int>(line.size()), m_Class
        );
    }
    ...
    namespace diagnostic_utils {
        void OutputLine(int lineNum, std::string const &line) {
            fmt::print(fmt::fg(c_ColorNeutral), "{:>4} ┃", lineNum);
    ...
    FatalCompilerError::OptionalData const &
    FatalCompilerError::GetData1() const noexcept {
        return m_Data1;
    }

    FatalCompilerError::OptionalData const &
    FatalCompilerError::GetData2() const noexcept {
        return m_Data2;
    }
